...
function Base.write(io::IO, pband::AbstractArray{<:Real, 3}, ::Type{PBAND_dat}; comment = "",
	kline = Kline(; line = collect(1:size(pband, 3)), name = ["Γ"], index = [1]),
	projection::AbstractVector{String} = String[])

	(Nband, np1, Nk) = size(pband)
	np = np1 - 1
	Nk == length(kline.line) || begin
	...
	tablehead = "#K-Path(1/A)    Energy(eV) "
	for p in projection
		tablehead *= @sprintf("%8s ", p)
	end
	tablehead *= @sprintf("%8s", "tot")


	println(io, comment)
	println(io, tablehead)
	...
	print(io, "\n# kpoint: ")
	for index in kline.index
		@printf(io, "%12.6f ", kline.line[index])
	end
	print(io, "\n# kname:  ")
	...
	if Nk == 1
		...
		pband = reshape(pband, Nband, np1)
		for i in 1:Nband
			output[i] = @sprintf("%8u %16.8f ", i, pband[i, 1])
			for j in 1:np
				output[i] *= @sprintf("%8.4f ", pband[i, j+1])
			end
			output[i] *= @sprintf("%8.4f", sum(pband[i, 2:end]))
		end
		writedlm(io, output)
	else
		line = Vector{String}(undef, Nk)
		for i in 1:Nk
			line[i] = @sprintf("%12.6f ", kline.line[i])
		end
		output = Vector{String}(undef, Nk)
		for i in 1:Nband
			for j in 1:Nk
				output[j] = @sprintf("%16.8f ", pband[i, 1, j])
				for k in 1:np
					output[j] *= @sprintf("%8.4f ", pband[i, k+1, j])
				end
				output[j] *= @sprintf("%8.4f", sum(pband[i, 2:end, j]))
			end
			println(io, "# Band-Index   $i")
			writedlm(io, line .* output)
			println(io, "")
		end
	end
...
"""
function WritePBAND(kdata, data::AbstractVector, orbital::AbstractVector{<:Pair}, file::AbstractString; mode = "w", comment = "From MyWrite.PBAND.")
	#orbital = ["name" => [index]].
...
#################################################################################################
function PBAND_write(kdata, band::AbstractMatrix{<:Real}, bandindex::AbstractVector{<:Integer}, orbcomp::AbstractArray{<:Real}, tablehead::AbstractString, file::AbstractString; mode = "w", comment = "From MyWrite.PBAND.")
